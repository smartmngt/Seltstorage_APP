...
async function checkLogin() {
  try {
    const r = await fetchWithTimeout(API + '/api/me', { credentials: 'include' }, 20000).then(r => r.json());
...
async function doLogin() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      errEl.textContent = d.error || `로그인 실패 (상태코드 ${res.status})`;
    }
  } catch (e) {
    errEl.textContent = '서버에 연결할 수 없어요: ' + e.message;
  }
}
