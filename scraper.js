...
async function withCupidRetry(makeRequest, baseCookie) {
  ...
  const redirectPath = findJsRedirect(res.data); // 예: ".../member_manager.php?ckattempt=1"

  const newCookie = (baseCookie ? baseCookie + '; ' : '') + 'CUPID=' + cupidVal;

  // 실제 브라우저처럼, 쿠키를 얻은 뒤 챌린지가 안내한 주소를 한번 GET으로 방문해줌
  if (redirectPath) {
    try {
      await axios.get(new URL(redirectPath, BASE).toString(), {
        headers: { ...BROWSER_HEADERS, Cookie: newCookie },
        validateStatus: () => true,
      });
      console.log('[cupid] 리다이렉트 주소 방문 완료, CUPID 쿠키 확정');
    } catch (e) {
      console.log('[cupid] 리다이렉트 방문 중 오류(무시하고 진행):', e.message);
    }
  }
  ...
  console.log('[cupid] 챌린지 통과, 실제 요청 완료');
